...
session or another person. Right now you are the best cold reader this document will ever
get, because you truly know nothing else. Use that before it wears off.

## Steps

1. **Find the handover.** Usually the user pastes it. If you can read files and the user
   points at one, read that. If there is no handover at all, say so, and ask what they
   would like to work on. Do not invent context.

2. **Read it cold, and report first.** Before doing any work, list what is unclear or
   missing:
   ...
   you or send you the wrong way.

3. **Play it back in three lines:** the goal, where things stand, and the next step as
   you understand it.

4. **Check it is still current.** If the handover carries a date and looks old, or
   mentions things that may have moved on, say so.

5. **Ask how to proceed:** continue from the next step, go through the full handover
   together, or start something different. Wait for the answer.

6. **Then work.** Once the user confirms, begin with the next step. Treat the constraints
   in the handover as standing rules for this whole conversation.

## Do not

- Do not start working before step 5 is answered.
